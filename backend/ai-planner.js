...
const roleplay = new PromptTemplate({
  template: `
     ...
     * Topic = {topic}
     ...
     4. If the estimated time is high , balance it across available days.
...
export async function generateStudyPlan(difficulty, deadline, est, type, studyHours, topic) {
  try {
    ...
    Provide the study plan in strict JSON format with clear sub-topic labels.In the example followed by I want you to replace the Day 1, Day 2 and so on to the actual dates, I want you to start from the day that the task has been assigned to the (n-1)th date that is the day before the deadline
